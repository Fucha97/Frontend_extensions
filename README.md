...
`-` В режиме работы этого расширения юзер может двигать мышкой и видеть цвет каждого пикселя, на котором в текущий момент установлен курсор. Рядом с курсором виден [label](./materials/label.png) в котором указан цвет пикселя в форматах HEX и RGB. \
